...
The list of variables declared at the top of ``rundat_mod.F90`` is also written out
to an auxiliary XML file that can help with restarting or continuing a ONETEP run,
as described in :ref:`restarting_onetep`. The code that handles the XML-related tasks
is auto-generated by a small program included in the ``rundat_new`` folder. For this to
work correctly, the following steps should be followed:
  1. Ensure that all variables declared in ``rundat_mod.F90`` have matching
     declarations in the file ``rundat_vars.F90`` within the folder ``rundat_new``.
     If some are missing or need correcting this has to be done by hand.
  2. Auto-generate the XML-related code by running the ``make_rundat_new.sh``
     script also given in the ``rundat_new`` folder. It may be necessary to manually break
     code lines that are longer than admissible by the coding standards (this will be fixed
     in the near future).
  3. Running the ``make_rundat_new.sh`` script also generates a comparison table between the
